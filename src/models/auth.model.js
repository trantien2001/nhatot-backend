...
const authModel = {
      ...
      (err, result) => {
        if (err) {
          ...
          if (bcrypt.compareSync(password, String(result[0]?.Password).trim())) {
            ...
            return res.status(200).send({ users, accessToken, message: 'Logged in successfully' });
          } else {
  ...
  register: async (req, res) => {
    ...
    await sql.query(`SELECT * FROM user WHERE PhoneNumber = ${phoneNumber}`, (err, result) => {
      if (err) {
    ...
    await sql.query(`SELECT * FROM user WHERE PhoneNumber = ${phoneNumber}`, (err, result) => {
      ...
      const { Active, Password, ...users } = result[0];
      return res.status(200).send({ users, message: 'User has been created' });
    });
  },
};
